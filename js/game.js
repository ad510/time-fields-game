...
function load(color) {
  ...
  fields[0] = player
  shots[0] = new Obj("img/clock.png", -200, 0, 0, 0, 0)
  shots[0].velRot = PlayerRotSpd
  shots[1] = new Obj("img/clock.png", 200, 0, 0, 0, Math.PI)
  shots[1].velRot = PlayerRotSpd
...
function update() {
  ...
  updateObjs()
  if (Math.abs(shots[0].rot - shots[1].rot) < 0.1) {
    document.getElementById("instruct").style.display = "none"
...
function updateObjs() {
  ...
  for (var i = 1; i < fields.length; i++) updatePos(fields[i], 0, 0)
...
function updatePos(obj, fwd, rot) {
  ...
  if (obj.rot > Math.PI * 2) obj.rot -= Math.PI * 2
